...
//
val range = 1..15
val map = range.map{t -> if (t%3==0 && t%5==0) "FIZZBUZZ" else if (t%3==0) "FIZZ" else if (t%5==0) "BUZZ" else ""}
val mapFoldResults = map.fold("", {mapFoldResults, next -> mapFoldResults+next})
...
val r2 = process("FOO", {_ -> r2_message.toUpperCase()+r2_message.toUpperCase()+r2_message.toUpperCase()}) // call process() with message "FOO" and a block that upper-cases 
